...
const connection = new IORedis(process.env.REDIS_URL!, {
  maxRetriesPerRequest: null,   // recomendado con Upstash
  enableReadyCheck: false,      // recomendado con Upstash
});
...
export const worker = (q: keyof typeof queues, handler: (d: any) => Promise<void>) =>
  new Worker(q as string, async (job: any) => handler(job.data), {
    connection,
    concurrency: 5,
  });
